...
This document maps functions from `clean_chems.py` to proposed R implementations in ChemReg,
...
The current ChemReg pipeline has a gap between **file upload/detection** and **curation (tiered CompTox search)**. The Python script `clean_chems.py` fills that gap with a staged cleaning pipeline. This plan proposes integrating that cleaning as two new modules:

```
File Upload → Detection → Extract Clean Data
                                ↓
                    ┌───────────────────────┐
                    │  R/pre_curation.R     │  ← NEW
                    │  (cleaning + flagging) │
                    └───────────────────────┘
                                ↓
                    Column Tagging → Curation Pipeline (existing R/curation.R)
                                ↓
                    ┌───────────────────────┐
                    │  R/post_curation.R    │  ← NEW
                    │  (QC + audit export)  │
                    └───────────────────────┘
                                ↓
                          Final Export
...
#### `extract_cas` — Extract CAS-RNs from text (CASRN or Name columns)

| | Python (`clean_chems.py`) | ComptoxR |
|---|---|---|
| **Function** | `casrn_split(x)` (line 7) | `ComptoxR::extract_cas(x)` |
| **Approach** | `re.findall("[1-9][0-9]{1,6}\-[0-9]{2}\-[0-9]")` | `str_extract_all` candidates → `as_cas()` validation |
...
**Where it goes:** `R/pre_curation.R` — called early to rescue CAS-RNs from name columns and split multi-CAS cells.

---

#### `validate_cas` — CAS-RN checksum validation
...
**Where it goes:** `R/pre_curation.R` — batch validation step with audit comment logging.

---

#### `normalize_cas` — CAS-RN normalization

| | Python | ComptoxR |
|---|---|---|
| **Function** | Manual `str.strip()` chain (lines 775-783) | `ComptoxR::as_cas(x)` |
| **Approach** | Strip whitespace, `...`, `#`, `*`, collapse spaces, remove spaces | Strip non-digits → reformat as `NNN-NN-N` → validate checksum |
...
- Only returns a value if the reconstructed CAS passes checksum — the Python version just strips characters without revalidating.
- Already used in `R/curation.R:202`.

**Where it goes:** `R/pre_curation.R` — first step in CASRN pipeline, before validation.

---

#### `rescue_cas_from_names` — Move CAS-RNs found in name columns to CASRN column
...
**Where it goes:** `R/pre_curation.R` — after initial string canonicalization, before name cleaning.

---

#### `split_multi_cas` — Explode rows with multiple CAS-RNs
...
**Where it goes:** `R/pre_curation.R` — after `rescue_cas_from_names`, before checksum validation.

---

#### `filter_invalid_cas` — Remove strings in CASRN column that aren't CAS-RNs

| | Python | ComptoxR |
|---|---|---|
| **Function** | `string_not_casrn()` (line 641) | `ComptoxR::as_cas()` returning `NA` |

**Recommendation: Use `ComptoxR::as_cas()` — returns `NA` for non-CAS strings automatically.**
...
**Where it goes:** `R/pre_curation.R` — implicit in the normalization step.

---

### 2.2 Chemical Name Cleaning

#### `detect_formula_names` — Flag names that are just molecular formulas
...
- **For the "is this name just a formula?" check**, `extract_formulas()` won't help — we need a different approach.
...
**Where it goes:** `R/pre_curation.R` — flag-only step (set name to `NA`, log original in comment).

---

#### `detect_mixture_names` — Flag names that indicate mixtures
...
**Recommendation: Use both — they catch different things.**
...
**Where it goes:** `R/pre_curation.R` — flag step, not removal. Let curator decide.

---

#### `strip_terminal_phrases` — Remove trailing parenthetical/bracket content
...
**Where it goes:** `R/pre_curation.R` — name canonicalization step.

---

#### `strip_unspecified` — Remove trailing ", unspecified" or "- unspecified"
...
#### `strip_salt_references` — Remove "and its salts" suffix
...
**Where it goes:** `R/pre_curation.R` — name canonicalization step, after terminal phrase stripping.

---

#### `strip_quality_adjectives` — Remove "pure", "tech grade", "chemical grade", etc.
...
#### `strip_text_noise` — Remove "Part A:", trailing percentages, "modified" names
...
1. `"Part [a-z]:"` prefix — split on colon, keep the chemical name portion
2. Terminal `\d+%` — strip trailing percentage
3. `"modif"` anywhere — flag entire name as ambiguous (set to `NA`)
...
These functions use reference lists to identify non-chemical names. In the Python pipeline, they *remove* names. In ChemReg, they should **flag** names for curator review rather than silently removing them.

#### `flag_functional_categories` — Flag names matching OECD/EPA functional use categories
...
#### `flag_food_names` — Flag names that are foods, not chemicals
...
#### `flag_stop_words` — Flag ambiguous names (proprietary, blend, inert, etc.)
...
#### `flag_block_list` — Flag known non-chemical entries
...
#### `fix_unicode` — Replace known Unicode code points with ASCII equivalents
...
| **Coverage** | `α`, `ω`, `®`, `–`, `°`, `'`, `…`, `'` | Full Greek alphabet, math symbols (±, ≥, ≤, ×, ÷), sub/superscripts, accented Latin chars, smart quotes, dashes, scientific units (µ, °) |
...
- No need for a separate `has_unicode()` check — `clean_unicode()` handles detection and replacement in one pass. For post-curation QC, `stringi::stri_enc_isascii()` can verify nothing remains.

**Where it goes:** `R/pre_curation.R` — very first step, before any regex matching.

---

#### `canonicalize_strings` — Strip leading/trailing whitespace and punctuation
...
**Where it goes:** `R/pre_curation.R` — applied at start and end of cleaning pipeline.
...
#### `post_validate_cas` — Re-validate resolved CAS-RNs
...
#### `post_detect_unicode` — Flag any remaining Unicode in final output
...
#### `post_functional_use_lookup` — Enrich resolved chemicals with functional use data
...
#### `post_safety_flags` — Add safety flag metadata
...
### `append_comment()` — Cross-cutting audit logging
...
────  ────────────────────────────  ────────────────  ─────────
 ...
 2    canonicalize_strings(name)    —                 No
 3    canonicalize_strings(cas)     —                 No
...
| (none) | `extract_mixture()` | Detects ratio-based mixtures (`60:40 w/w`) — new capability |
...
| (none — post-curation) | `chemi_functional_use()` | OECD functional use enrichment via DTXSID |
| (none — post-curation) | `chemi_resolver_safety_flags_bulk()` | Safety flag enrichment via DTXSID |
| (none — post-curation) | `chemi_resolver_lookup_bulk()` | Batch chemical resolution |
...
2. **Audit trail viewer** — expandable per-row comment history showing each transformation
3. **Toggle switches** — let user enable/disable individual cleaning steps
4. **Summary cards** — counts of: CAS-RNs rescued from names, invalid CAS-RNs removed, formulas detected, functional categories flagged, stop words flagged, block list entries removed
5. **Flagged rows table** — rows flagged (not removed) for curator review before curation begins
...
|---|---|---|
| Functional category list (OECD/EPA) | `ChemExpo_FC_2023-05-24.csv` | **Not in repo** — needs sourcing |
...
| Unicode mapping table | Internal to `ComptoxR::clean_unicode()` | **No porting needed** — 100+ mappings built in |
...
2. **Flag vs. Remove:** The Python script removes entries outright. ChemReg should flag for review (more conservative). Should flagged entries still flow into the curation pipeline, or be held back?
...
5. **`extract_mixture()` scope:** Currently only detects ratio patterns. Should we expand it with keyword matching ("blend", "mixture", etc.) directly in ComptoxR, or keep that as ChemReg-specific logic?

---

## 10. Real Data Analysis — `uncurated_chemicals_2023-05-16_12-43-41.csv`
...
1. **Comma-separated synonym splitting** — The biggest cleaning gap. Over 1,000 rows have `"name1, name2, name3"` packed into one field. Neither `clean_chems.py` nor ComptoxR handles this. Tricky because commas also appear inside IUPAC names (e.g., `"2,2-dimethylpropane"`).
2. **Semicolon-separated synonyms** — Less common but same splitting problem.
3. **Hazard warning stripping** — `term_parenth()` catches some terminal parentheticals, but embedded warnings like `"(suspected human carcinogen by ACGIH, NTP)"` need explicit handling.
4. **Extraction artifact cleanup** — Trailing underscores, asterisks around numbers (`*97-3*`).
5. **"No CAS" placeholder detection** — Text like `no cas`, `none`, `n/a` in the CAS column.
...
- Spec: `append_comment(existing, new_text, reason, sep = " | ")`
  - `existing = NA`, `new_text = "foo"`, `reason = "bar"` → `"bar: foo"`
  - `existing = "prev"`, `new_text = "foo"`, `reason = "bar"` → `"prev | bar: foo"`
  - `new_text = NA` → return `existing` unchanged
- Tests:
  - [x] NA existing + non-NA text → creates new comment
  - [x] Existing comment + non-NA text → appends with separator
  - [x] NA new_text → returns existing unchanged
  - [x] Both NA → returns NA
  - [x] Vectorized over a column

**P1.2: Unicode cleaning** — Use `ComptoxR::clean_unicode()`
...
- Tests:
  - [x] Greek letters → `.alpha.` etc.
  - [x] Smart quotes → ASCII quotes
...
- Tests:
  - [x] `"  Acetone  "` → `"Acetone"`
  - [x] `"#Toluene#"` → `"Toluene"`
  - [x] `"aerothane tt_"` → `"aerothane tt"`
  - [x] `"Sodium  chloride"` → `"Sodium chloride"`
  - [x] Preserves internal parentheses: `"Iron(III) chloride"` unchanged

---

### Phase 2 — CAS-RN Pipeline (14.4% + 2.4% of rows)

**P2.1: CAS placeholder detection** `R/pre_curation.R`
> New function — not in `clean_chems.py`. Handles ~296 "no cas" rows.
...
**P2.2: CAS normalization** — Use `ComptoxR::as_cas()`
...
**P2.3: CAS extraction from names** — Use `ComptoxR::extract_cas()`
...
- Wrapper needed: `rescue_cas_from_names(df, name_col, cas_col, comment_col)`
  ...
  - Logs to comment
- Tests:
  - [x] `"ethanol (CAS 64-17-5)"` → name becomes `"ethanol"`, CAS becomes `"64-17-5"`
  - [x] Name with no CAS → unchanged
  - [x] CAS already in CAS column → no duplicate
...
- Tests:
  - [x] Single CAS → 1 row
  - [x] Two CAS-RNs → 2 rows, comment logged
  - [x] NA CAS → 1 row, unchanged

**P2.5: CAS checksum validation** — Use `ComptoxR::is_cas()`
...
### Phase 3 — Name Cleaning Core (8%+ of rows — synonym splitting)

**P3.1: Hazard warning stripping** `R/pre_curation.R`
> New function — critical because warnings poison synonym splitting and curation lookups.
...
- Tests:
  - [x] `"ethylene oxide (suspected 2a human carcinogen by iarc)"` → `"ethylene oxide"`
  - [x] `"carbon black (suspected human carcinogen by ACGIH)"` → `"carbon black"`
  - [x] `"Iron(III) chloride"` → unchanged (not a warning)
...
- Tests:
  - [x] `"Acetone (ACS reagent)"` → `"Acetone"`, comment: `"ACS reagent"`
  - [x] `"dimethyl (methyl)"` → unchanged (contains "yl")
  - [x] `"compound (high density)"` → `"compound"` (false positive "yl" in "density")

**P3.3: Synonym splitting** `R/pre_curation.R`
> **NEW — not in `clean_chems.py`.** Biggest gap for the live dataset.

- Spec: `split_synonyms(name_string)` → returns primary name + vector of synonyms
- Strategy (ordered):
  1. Split on semicolons first (least ambiguous): `"acetone; dimethyl ketone"` → `["acetone", "dimethyl ketone"]`
...
- **Critical edge cases from live data:**
  - `"2-butoxyethanol (ethyleneglycol monobutyl ether), butyl cellosolve"` — comma after parenthetical closes the first synonym
  - `"butane, 2,2-dimethyl"` — this is ONE name (IUPAC inverted form), not two
  - `"Hydrocarbons, C12-C16, isoalkanes, cyclic <2% aromatics"` — classification descriptor, not synonyms
- This is the hardest function. May need iterative refinement.
- Tests:
  - [x] `"xylene, dimethylbenzene, xylol"` → 3 names
  - [x] `"butane, 2,2-dimethyl"` → 1 name (IUPAC inverted)
  - [x] `"acetone; dimethyl ketone"` → 2 names
  - [x] `"1,4-Dioxane"` → 1 name (comma inside number)
  - [x] Simple name with no separators → 1 name

---

### Phase 4 — Reference Data Filters (2.5% + 2.2% of rows)
...
- Tests:
  - [x] `"Flavor"` → flagged
  - [x] `"PARFUM/FRAGRANCE"` → flagged
  - [x] `"yellow colorant"` → flagged
  - [x] `"sodium chloride"` → not flagged
  - [x] `"methylparaben"` → not flagged (contains chemical suffix)
...
- Tests:
  - [x] `"proprietary blend"` → flagged (stop word)
  - [x] `"trade secret"` → flagged
  - [x] `"polymer"` → flagged
  - [x] `"methyl ethyl ketone"` → NOT flagged (`"yl"` exception)
  - [x] `"Ethanol, water (1:1)"` → flagged by `extract_mixture()`
...
- Tests:
  - [x] `"alcohol"` → flagged
  - [x] `"Acrylic Polymer"` → flagged
  - [x] `"acetone"` → NOT flagged

---

### Phase 5 — Name Cleaning Detail (Smaller issue counts but improves curation hit rate)
...
- Tests:
  - [x] `"Acetone (99.5%)"` → after P3.2 strips parens; this strips remaining "grade" text
  - [x] `"technical grade ethanol"` → `"ethanol"`, comment logs `"technical grade"`

**P5.2: "Unspecified" and salt reference stripping** `R/pre_curation.R`
> Port `terminal_unspecified()` and `drop_salts()`.

- Tests:
  - [x] `"acid, unspecified"` → `"acid"`, comment logged
  - [x] `"lead and its salts"` → `"lead"`, comment logged

**P5.3: Formula-as-name detection** `R/pre_curation.R`
> Uses `ComptoxR::pt` data to validate elements.

- Spec: `is_formula_name(name_string)` → logical
...
- Tests:
  ...
  - [x] `"C2H5OH"` → TRUE
  - [x] `"water"` → FALSE
  - [x] `"H2O and stuff"` → FALSE (not bare formula)
...
### Phase 6 — Post-Curation QC

**P6.1: Post-validation CAS check** — `ComptoxR::is_cas()`
**P6.2: Remaining Unicode check** — `stringi::stri_enc_isascii()`
**P6.3: Functional use enrichment** — `ComptoxR::chemi_functional_use()`
**P6.4: Safety flag enrichment** — `ComptoxR::chemi_resolver_safety_flags_bulk()`

---

## 12. ComptoxR Adjustments Needed

### 12.1 `extract_mixture()` — Expand to keyword detection
...
- Tests:
  - [x] `"Ethanol, water (1:1)"` → TRUE (ratio, same as before)
  - [x] `"mixture of acids"` → FALSE with default, TRUE with `include_keywords = TRUE`
  - [x] `"sodium chloride"` → FALSE in both modes
  - [x] `"compounded rubber"` → TRUE with keywords (word boundary prevents matching "compound" inside chemical names like "organic compound")

### 12.2 `extract_formulas()` — No changes needed
...
### 12.3 `clean_unicode()` — Evaluate for chemical-specific additions
...
### 12.4 `as_cas()` — Evaluate stricter non-CAS detection

**Current behavior:** Strips non-digits, reformats, validates.
**Potential issue:** Strings like `"no cas"` contain digits? No — `"no cas"` has no digits so `as_cas()` already returns `NA`. But strings like `"CAS: 7732-18-5"` will correctly extract `7732-18-5`. Current behavior is correct.
...
3. Also allow: hydrate notation (`·`, `.`), charge notation (`+`, `-`), grouped substructures with parentheses
...
| `"Iron(III) chloride"` | `character(0)` — oxidation state, not warning |
| `"acetone"` | `character(0)` |
| `"compound (toxic to aquatic life)"` | `"toxic to aquatic life"` |

---

## 15. Functions to Port to ChemReg (Not Thematically Correct for ComptoxR)
...
### 15.1 `append_comment()` — Audit trail helper

See P1.1 above. ChemReg-specific infrastructure.

### 15.2 `canonicalize_strings()` — Generic string cleanup

See P1.3 above. Too generic for a chemistry package.

### 15.3 `detect_cas_placeholders()` — "No CAS" detection

See P2.1 above. Application-specific (handles data entry conventions, not chemistry).

### 15.4 `split_synonyms()` — Comma/semicolon synonym splitting

See P3.3 above. Highly context-dependent heuristic, not generalizable.

### 15.5 `strip_quality_adjectives()` — Remove "tech grade", "pure", etc.

See P5.1 above. Domain-specific text munging.

### 15.6 `strip_unspecified()` — Remove trailing ", unspecified"

See P5.2 above. Trivial regex, app-specific.

### 15.7 `strip_salt_references()` — Remove "and its salts"

See P5.2 above. Narrow regex, app-specific.

### 15.8 Reference data lists — Stop words, block list, food names, functional categories
...
cleaning_functional_categories <- function() {
  # Core keywords — expandable. Does NOT require ChemExpo CSV.
...
### 15.9 `strip_terminal_enclosures()` — Remove trailing `(...)` or `[...]`

See P3.2 above. The `"yl"` heuristic is ChemReg-specific domain knowledge, not generalizable.
...
| Issue | Covered? | Gap |
|-------|----------|-----|
| CAS format validation | Yes (13 records) | — |
| CAS checksum | Yes (4 records) | — |
| Name-CAS mismatch | Yes (4 records) | — |
| Missing data | Yes (3 records) | — |
| Typos in names | Yes (3 records) | — |
| Special chars in names | Yes (2 records) | — |
| Whitespace issues | Yes (2 records) | — |
| Quotes/semicolons/pipes | Yes (3 records) | — |
| Names with qualifiers | Yes (7 records) | — |
...
| **Guaranteed API misses** | Yes (3 records) | — |
