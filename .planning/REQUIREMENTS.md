...
- [ ] **PARS-01**: Normalization chain — whitespace removal, `x10^` → `e`, Fortran exponents (`4.56+02`), comma stripping
- [ ] **PARS-02**: Qualifier extraction (`<`, `>`, `<=`, `>=`, `~`) before any range splitting
...
- [ ] **PARS-04**: Parse result tibble structure: `numeric_value`, `qualifier`, `range_bin` (as_is/low/mid/high), `parse_flag`
- [ ] **PARS-05**: `orig_result` capture as absolute first step before any transformation for audit trail
...
| PARS-04 | Phase 30 | Pending |
| PARS-05 | Phase 30 | Pending |
