...
| 2 | Pipeline Config sheet contains concert_export marker set to 'true' | ✓ VERIFIED | Marker verified in tests (tests/test_export_import.R:295-316), key="concert_export", value="true" |
...
| 4 | Config import detects CONCERT exports and rejects non-CONCERT files | ✓ VERIFIED | Two-stage validation tested (tests/test_export_import.R:376-427): valid exports return non-NULL, non-CONCERT files return NULL |
...
| 9 | User can upload a CONCERT export via sidebar config import control | ✓ VERIFIED | fileInput wired in sidebar (app.R:81-88), accept=".xlsx", placeholder="CONCERT export (.xlsx)" |
| 10 | User sees confirmation modal with checkboxes when valid CONCERT export detected | ✓ VERIFIED | modalDialog implemented (app.R:167-180), two checkboxInput controls for restore_ref_lists and restore_col_tags |
...
| 12 | Non-CONCERT Excel files show warning notification and do not trigger modal | ✓ VERIFIED | NULL check implemented (app.R:154-161), showNotification with type="warning", return() prevents modal |
...
| R/config_import.R | parse_concert_export() and merge_reference_lists() functions | ✓ VERIFIED | 110 lines, exports both functions, two-stage validation logic |
...
| app.R | R/config_import.R | parse_concert_export() in observeEvent | ✓ WIRED | Line 152, result stored in imported_config() reactiveVal, NULL check triggers warning |
...
| EXPO-02 | 14-01, 14-02 | User can re-import a CONCERT export and see a confirmation modal offering to restore embedded reference lists and pipeline state | ✓ SATISFIED | parse_concert_export() validates exports (config_import.R:17-64), modal with checkboxes implemented (app.R:167-180), 3 tests pass (test_export_import.R:376-427) |
| EXPO-03 | 14-01, 14-02 | User can see the multi-sheet export serve as both a standalone audit document and a CONCERT re-entry point | ✓ SATISFIED | Pipeline Config contains concert_export marker (export_helpers.R:98-119), merge_reference_lists() preserves/imports entries (config_import.R:76-109), 6 tests pass covering audit metadata and merge logic |
...
- Pipeline Config sheet contains key-value pairs including concert_export=true marker
...
**Expected:**
- Modal appears with title "CONCERT Export Detected"
...
#### 3. Non-CONCERT File Rejection

**Test:** Upload a random Excel file (.xlsx) that is NOT a CONCERT export (e.g., a basic spreadsheet with data) via the "Import Configuration" file input in the sidebar

**Expected:**
- Warning notification appears: "Not a valid CONCERT export file. Upload a file exported from CONCERT with Pipeline Config sheet."
...
**Test Coverage:**
- Backend functions: 57 unit tests (build_export_sheets, validate_excel_size, parse_concert_export, merge_reference_lists)
- Integration: downloadHandler wired, config import wired, data_store updates wired
- Edge cases: NULL cleaning_audit handled, non-CONCERT files rejected, oversized data blocked
