...
**Success Criteria** (what must be TRUE):
  1. Developer runs `devtools::install()` and then `library(concert)` without any errors or warnings about missing fields
...
**Goal**: The Shiny app lives under inst/app/ and is launchable via concert::run_app() after package install
**Depends on**: Phase 25
**Requirements**: APP-01, APP-02, APP-03, APP-04, APP-05
**Success Criteria** (what must be TRUE):
  1. `inst/app/app.R` exists and the project root no longer contains `app.R`
  2. `concert::run_app()` launches the Shiny app without errors after `devtools::install()`
  3. Reference cache paths use `system.file("extdata", "reference_cache", package = "concert")` — no `here::here()` calls remain in package source files
