...
result: FAILED — WQX confidence column is duplicated in the table
...
result: FAILED — Search box does not work for finding a new WQX term
...
### GAP-1: WQX confidence column duplicated
status: failed
description: The WQX Conf. column appears twice in the Review Results table. Likely caused by the grep-based colDef guard matching both the original and suffixed column name, creating two colDefs that both render.

### GAP-2: WQX Review modal search box broken
status: failed
description: The type-ahead search box in the WQX Review modal does not work for finding a new WQX term. The override workflow cannot be completed.
