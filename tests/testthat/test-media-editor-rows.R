...
make_editor_media_map <- function() {
  tibble::tibble(
    term = c("water", "runoff", "soil"),
    canonical = c("water", NA_character_, "soil"),
    canonical_term = c("water", NA_character_, "soil"),
    envo_id = c("ENVO:00002006", NA_character_, "ENVO:00001998"),
    media_category = c("aqueous", NA_character_, "solid"),
    source = c("concert", "amos", "user"),
    assertion_mode = c("auto", "pending", "user"),
    confidence = c("high", "pending", "user"),
    active = TRUE
  )
}
...
test_that("build_media_editor_rows does not duplicate existing mappings with raw unmatched rows", {
  media_results <- tibble::tibble(
    raw_media = c("water", "water"),
    media_flag = rep("media_unmatched", 2)
  )
  ...
  expect_equal(water$canonical, "water")
  expect_equal(water$unmatched_count, 2L)
})
