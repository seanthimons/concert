...
mock_wqx_resolves_arsenic <- function(
  names_vec,
  dict,
  threshold = 0.85,
  ...
...
mock_wqx_resolves_none <- function(
  names_vec,
  dict,
  threshold = 0.85,
  ...
...
# Helper to mock pipeline dependencies at namespace level
with_pipeline_mocks <- function(
  mock_match_wqx,
  mock_sw = empty_sw,
  code
) {
  ...
  originals <- lapply(fns, function(f) ns[[f]])
  names(originals) <- fns
  for (f in fns) {
    unlockBinding(f, ns)
  }
  ...
  assign("search_exact", empty_exact, envir = ns)
  assign("validate_and_lookup_cas", empty_cas, envir = ns)
  assign("load_wqx_dictionary", function(...) mock_wqx_dict, envir = ns)
  assign("match_wqx", mock_match_wqx, envir = ns)
  assign("search_starts_with", mock_sw, envir = ns)
...
test_that("WQX tier resolves names before starts-with (ORD-01)", {
  ...
  with_pipeline_mocks(
    mock_match_wqx = mock_wqx_resolves_arsenic,
    code = {
      ...
      tiers <- result$results$source_tier
...
test_that("wqx_threshold parameter passes through to match_wqx (CONF-02)", {
  ...
  with_pipeline_mocks(
    mock_match_wqx = function(names_vec, dict, threshold = 0.85, ...) {
      captured_threshold <<- threshold
      mock_wqx_resolves_none(names_vec, dict, threshold, ...)
    },
    code = {
...
test_that("starts_with=FALSE skips starts-with entirely (TOG-02)", {
  ...
  with_pipeline_mocks(
    mock_match_wqx = mock_wqx_resolves_none,
    mock_sw = function(names_vec, ...) {
      sw_called <<- TRUE
      empty_sw()
    },
    code = {
      ...
      expect_false(
        "starts_with" %in% result$results$source_tier
      )
    }
  )
})
