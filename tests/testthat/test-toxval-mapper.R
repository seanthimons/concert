...
test_that("toxval schema has no logical columns (no bare NAs)", {
  ...
  expect_equal(length(logical_cols), 0, info = paste("Found logical columns:", paste(logical_cols, collapse = ", ")))
})
...
test_that("consensus_name overrides name for non-DTXSID (WQX) rows only", {
  # Row 1: WQX-resolved, no DTXSID -> name should become the canonical consensus_name.
  # Row 2: DTXSID present -> name should keep the raw text, ignoring consensus_name.
  curated <- tibble::tibble(
    consensus_dtxsid = c(NA_character_, "DTXSID7020182"),
    chemical_name = c("DO", "Benzene"),
    consensus_name = c("Dissolved oxygen", "should-be-ignored")
  )
  result <- map_to_toxval_schema(curated, harmonized_fixture)

  expect_true(is.na(result$dtxsid[1]))
  expect_equal(result$name[1], "Dissolved oxygen")
  expect_equal(result$name[2], "Benzene")
})

test_that("name falls back to raw text when consensus_name is NA", {
  curated <- tibble::tibble(
    consensus_dtxsid = c(NA_character_, NA_character_),
    chemical_name = c("Foo", "Bar"),
    consensus_name = c(NA_character_, NA_character_)
  )
  result <- map_to_toxval_schema(curated, harmonized_fixture)

  expect_equal(result$name, c("Foo", "Bar"))
})
...
test_that("source set to custom value when source_name provided", {
  result <- map_to_toxval_schema(curated_fixture, harmonized_fixture, source_name = "ECOTOX")
